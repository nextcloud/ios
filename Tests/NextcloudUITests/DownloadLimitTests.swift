...
final class DownloadLimitTests: XCTestCase {
    ...
    func logIn() throws {
        ...
        // The login is taking place in the default browser of the device.
        // Hence we also need to remote control Safari.
        // Should the app use an in-app SFSafariViewController again, the web view can be queried directly.

        let safari = XCUIApplication(bundleIdentifier: "com.apple.mobilesafari")
        safari.await()

        guard safari.webViews.firstMatch.await() else {
            XCTFail("Expected Safari web view did not appear in time!")
            return
        }

        guard safari.state == .runningForeground else {
            XCTFail("Safari is not running in foreground!")
            return
        }
        ...
        guard grantButton.await() else {
            throw UITestError.waitForExistence
        }
