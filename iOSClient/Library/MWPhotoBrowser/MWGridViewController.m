...
{
    NSIndexPath *currentPhotoIndexPath = [NSIndexPath indexPathForItem:index inSection:0];
    NSArray *visibleIndexPaths = [self.collectionView indexPathsForVisibleItems];
    BOOL currentVisible = NO;
    for (NSIndexPath *indexPath in visibleIndexPaths) {
        if ([indexPath isEqual:currentPhotoIndexPath]) {
...
{
    NSIndexPath *currentPhotoIndexPath = [NSIndexPath indexPathForItem:index inSection:0];
    NSArray *indexpathArray = [[NSArray alloc] initWithObjects:currentPhotoIndexPath, nil];
