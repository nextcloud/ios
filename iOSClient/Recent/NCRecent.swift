...
class NCRecent: NCCollectionViewCommon {
    ...
    override func queryDB() {
        ...
        let metadatas = NCManageDatabase.shared.getMetadatas(predicate: NSPredicate(format: "account == %@", self.userBaseUrl.account), numItems: 200, sorted: "date", ascending: false)

        layoutForView?.sort = "date"
        layoutForView?.ascending = false
        layoutForView?.directoryOnTop = false

        self.dataSource = NCDataSource(metadatas: metadatas, account: self.userBaseUrl.account, layoutForView: layoutForView, favoriteOnTop: false, providers: self.providers, searchResults: self.searchResults)
    }
