...
@objc class NCKeychain: NSObject {
    ...
    func isEndToEndEnabled(account: String) -> Bool {
        guard let capabilities = NCCapabilities.shared.getCapabilitiesBlocking(for: account),
              ...
              NCGlobal.shared.e2eeVersions.contains(capabilities.capabilityE2EEApiVersion) else {
            return false
        }
        return true
    }
