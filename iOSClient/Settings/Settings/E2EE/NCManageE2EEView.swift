...
struct NCManageE2EEView: View {
    ...
    var deleteCerificateSection: some View {
        Section(header: Text("Delete Server keys"), footer: Text("Available only in debug mode")) {
            ...
            .contentShape(Rectangle())
            .onTapGesture {
                NextcloudKit.shared.deleteE2EECertificate(account: NCDomain.shared.getActiveAccount()) { _, error in
                    if error == .success {
            ...
            .contentShape(Rectangle())
            .onTapGesture {
                NextcloudKit.shared.deleteE2EEPrivateKey(account: NCDomain.shared.getActiveAccount()) { _, error in
                    if error == .success {
