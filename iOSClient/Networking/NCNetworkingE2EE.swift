...
@objc class NCNetworkingE2EE: NSObject {
    ...
    func createFolder(fileName: String, serverUrl: String, account: String, urlBase: String, completion: @escaping (_ error: NCCError) -> Void) {
        ...
        self.lock(account: account, serverUrl: serverUrl) { directory, e2eToken, error in
            if error.errorCode == 0 && e2eToken != nil && directory != nil {

                NCCommunication.shared.createFolder(fileNameFolderUrl, addCustomHeaders: ["e2e-token": e2eToken!]) { account, ocId, _, error in
                    if error.errorCode == 0 {
                        guard let fileId = NCUtility.shared.ocIdToFileId(ocId: ocId) else {
                            ...
                            if let tableLock = NCManageDatabase.shared.getE2ETokenLock(account: account, serverUrl: serverUrl) {
                                NCCommunication.shared.lockE2EEFolder(fileId: tableLock.fileId, e2eToken: tableLock.e2eToken, method: "DELETE") { _, _, _ in }
                            }
                            return completion(.internal("Error convert ocId"))
                        }
                        NCCommunication.shared.markE2EEFolder(fileId: fileId, delete: false) { account, error in
                            if error.errorCode == 0 {
    ...
    func deleteMetadata(_ metadata: tableMetadata, completion: @escaping (_ error: NCCError) -> Void) {
    ...
    func renameMetadata(_ metadata: tableMetadata, fileNameNew: String, completion: @escaping (_ error: NCCError) -> Void) {
        ...
        if NCManageDatabase.shared.getE2eEncryption(predicate: NSPredicate(format: "account == %@ AND serverUrl == %@ AND fileName == %@", metadata.account, metadata.serverUrl, fileNameNew)) != nil {
            ...
            completion(.internal("_file_already_exists_"))
    ...
    func upload(metadata: tableMetadata, start: @escaping () -> Void, completion: @escaping (_ error: NCCError) -> Void) {
        ...
        if metadata.size > NCGlobal.shared.e2eeMaxFileSize {
            ...
            start()
            return completion(.internal("E2E Error file too big"))
        }
        ...
        if NCEndToEndEncryption.sharedManager()?.encryptFileName(metadata.fileNameView, fileNameIdentifier: metadata.fileName, directory: CCUtility.getDirectoryProviderStorageOcId(metadata.ocId), key: &key, initializationVector: &initializationVector, authenticationTag: &authenticationTag) == false {
            ...
            start()
            return completion(.internal("_e2e_error_create_encrypted_"))
        }
        ...
        NCNetworkingE2EE.shared.sendE2EMetadata(account: metadata.account, serverUrl: serverUrl, fileNameRename: nil, fileNameNewRename: nil, deleteE2eEncryption: nil, urlBase: metadata.urlBase, upload: true) { e2eToken, error in
            ...
            if error.errorCode == 0 && e2eToken != nil {
                ...
                NCCommunication.shared.upload(serverUrlFileName: serverUrlFileName, fileNameLocalPath: fileNameLocalPath, dateCreationFile: metadata.date as Date, dateModificationFile: metadata.date as Date, addCustomHeaders: ["e2e-token": e2eToken!], requestHandler: { request in
                    ...
                    if let metadata = NCManageDatabase.shared.getMetadataFromOcId(metadata.ocId) {
                        ...
                        } else if error.errorCode == 0 && ocId != nil {
                ...
                if let metadata = NCManageDatabase.shared.getMetadataFromOcId(ocIdTemp) {
                    ...
                    NotificationCenter.default.postOnMainThread(name: NCGlobal.shared.notificationCenterUploadedFile, userInfo: ["ocId": metadata.ocId, "ocIdTemp": ocIdTemp, "errorCode": error.errorCode, "errorDescription": error.errorDescription])
                }
                completion(error)
            }
        }
    ...
    @objc func lock(account: String, serverUrl: String, completion: @escaping (_ direcrtory: tableDirectory?, _ e2eToken: String?, _ error: NCCError) -> Void) {

        var e2eToken: String?

        guard let directory = NCManageDatabase.shared.getTableDirectory(predicate: NSPredicate(format: "account == %@ AND serverUrl == %@", account, serverUrl)) else {
            return completion(nil, nil, .success)
        }
        ...
        NCCommunication.shared.lockE2EEFolder(fileId: directory.fileId, e2eToken: e2eToken, method: "POST") { account, e2eToken, error in
            if error.errorCode == 0 && e2eToken != nil {
    ...
    @objc func unlock(account: String, serverUrl: String, completion: @escaping (_ direcrtory: tableDirectory?, _ e2eToken: String?, _ error: NCCError?) -> Void) {
        ...
        NCCommunication.shared.lockE2EEFolder(fileId: directory.fileId, e2eToken: e2eToken, method: "DELETE") { account, e2eToken, error in
            if error.errorCode == 0 {
    ...
    @objc func sendE2EMetadata(account: String, serverUrl: String, fileNameRename: String?, fileNameNewRename: String?, deleteE2eEncryption: NSPredicate?, urlBase: String, upload: Bool = false, completion: @escaping (_ e2eToken: String?, _ error: NCCError) -> Void) {

        self.lock(account: account, serverUrl: serverUrl) { directory, e2eToken, error in
            if error.errorCode == 0 && e2eToken != nil && directory != nil {

                NCCommunication.shared.getE2EEMetadata(fileId: directory!.fileId, e2eToken: e2eToken) { account, e2eMetadata, error in
                    var method = "POST"
                    ...
                    if error.errorCode == 0 && e2eMetadata != nil {
                        if !NCEndToEndMetadata.shared.decoderMetadata(e2eMetadata!, privateKey: CCUtility.getEndToEndPrivateKey(account), serverUrl: serverUrl, account: account, urlBase: urlBase) {
                            return completion(e2eToken, NCCError.internal("_e2e_error_encode_metadata_"))
                        }
                        method = "PUT"
                    }
                    ...
                    NCCommunication.shared.putE2EEMetadata(fileId: directory!.fileId, e2eToken: e2eToken!, e2eMetadata: e2eMetadataNew, method: method) { account, _, error in
