...
extension NCEndToEndMetadata {
    ...
    func encodeMetadataV20(serverUrl: String, ocIdServerUrl: String, addUserId: String?, addCertificate: String?, removeUserId: String?, domain: NCDomain.Domain) -> (metadata: String?, signature: String?, counter: Int, error: NKError) {

        guard let directoryTop = utilityFileSystem.getDirectoryE2EETop(serverUrl: serverUrl, account: domain.account), let certificate = NCKeychain().getEndToEndCertificate(account: domain.account) else {
            return (nil, nil, 0, NKError(errorCode: NCGlobal.shared.errorUnexpectedResponseFromDB, errorDescription: "_e2e_error_"))
        }

        let isDirectoryTop = utilityFileSystem.isDirectoryE2EETop(account: domain.account, serverUrl: serverUrl)
        ...
        if isDirectoryTop {
            ...
            if let tableUserId = NCManageDatabase.shared.getE2EUser(account: domain.account, ocIdServerUrl: directoryTop.ocId, userId: domain.userId),
               let metadataKey = tableUserId.metadataKey {
                key = metadataKey
            } else {
                addUser(userId: domain.userId, certificate: certificate, key: key)
            }
            // ADDUSERID
            if let addUserId {
            ...
            // REMOVEUSERID
            if let removeUserId {
                NCManageDatabase.shared.deleteE2EUsers(account: domain.account, ocIdServerUrl: ocIdServerUrl, userId: removeUserId)
            }
            // FOR SECURITY recreate all users with key
            if let users = NCManageDatabase.shared.getE2EUsers(account: domain.account, ocIdServerUrl: ocIdServerUrl) {
                for user in users {
            ...
            guard let tableUserId = NCManageDatabase.shared.getE2EUser(account: domain.account, ocIdServerUrl: directoryTop.ocId, userId: domain.userId), let key = tableUserId.metadataKey else {
                return (nil, nil, 0, NKError(errorCode: NCGlobal.shared.errorUnexpectedResponseFromDB, errorDescription: "_e2e_error_"))
            }
        ...
        // CHECKSUM
        //
        if let users = NCManageDatabase.shared.getE2EUsers(account: domain.account, ocIdServerUrl: directoryTop.ocId) {
            for user in users {
                if isDirectoryTop {
        ...
        // COUNTER + 1
        //
        if let resultCounter = NCManageDatabase.shared.getCounterE2eMetadata(account: domain.account, ocIdServerUrl: ocIdServerUrl) {
            counter = resultCounter + 1
        }

        // CIPERTEXT
        //
        let e2eEncryptions = NCManageDatabase.shared.getE2eEncryptions(predicate: NSPredicate(format: "account == %@ AND serverUrl == %@", domain.account, serverUrl))
        ...
        do {
            ...
            let signature = createSignature(metadata: metadataCodable, users: usersCodable, version: NCGlobal.shared.e2eeVersionV20, certificate: certificate, domain: domain)
    ...
    func decodeMetadataV20(_ json: String, signature: String?, serverUrl: String, ocIdServerUrl: String, domain: NCDomain.Domain) -> NKError {

        guard let data = json.data(using: .utf8),
              let directoryTop = utilityFileSystem.getDirectoryE2EETop(serverUrl: serverUrl, account: domain.account) else {
        ...
        func addE2eEncryption(fileNameIdentifier: String, filename: String, authenticationTag: String, key: String, initializationVector: String, metadataKey: String, mimetype: String) {
            ...
            if let metadata = NCManageDatabase.shared.getMetadata(predicate: NSPredicate(format: "account == %@ AND fileName == %@", domain.account, fileNameIdentifier)) {

                let object = tableE2eEncryption.init(account: domain.account, ocIdServerUrl: ocIdServerUrl, fileNameIdentifier: fileNameIdentifier)
        ...
        do {
                ...
                if let users {
                    for user in users {
                        ...
                        if let encryptedMetadataKey = user.encryptedMetadataKey {
                            ...
                            if let decrypted = NCEndToEndEncryption.shared().decryptAsymmetricData(data, privateKey: NCKeychain().getEndToEndPrivateKey(account: domain.account)) {
                                metadataKey = decrypted
                            }
                        }
                        NCManageDatabase.shared.addE2EUsers(account: domain.account, serverUrl: serverUrl, ocIdServerUrl: ocIdServerUrl, userId: user.userId, certificate: user.certificate, encryptedMetadataKey: user.encryptedMetadataKey, metadataKey: metadataKey)
                    }
                }
            }
            ...
            //
            guard let tableUser = NCManageDatabase.shared.getE2EUser(account: domain.account, ocIdServerUrl: directoryTop.ocId, userId: domain.userId),
            ...
            //
            if let signature {
                if !verifySignature(account: domain.account, signature: signature, userId: tableUser.userId, metadata: metadata, users: users, version: version, certificate: tableUser.certificate) {
            ...
            if let filesdrop {
                for filedop in filesdrop {
                    ...
                    for user in filedop.value.users where user.userId == domain.userId {
                        let data = Data(base64Encoded: user.encryptedFiledropKey)
                        if let decryptedFiledropKey = NCEndToEndEncryption.shared().decryptAsymmetricData(data, privateKey: NCKeychain().getEndToEndPrivateKey(account: domain.account)) {
            ...
            if let resultCounter = NCManageDatabase.shared.getCounterE2eMetadata(account: domain.account, ocIdServerUrl: ocIdServerUrl) {
                ...
                if jsonCiphertextMetadata.counter < resultCounter {
                    ...
                    NCManageDatabase.shared.updateCounterE2eMetadata(account: domain.account, ocIdServerUrl: ocIdServerUrl, counter: jsonCiphertextMetadata.counter)
                }
            } else {
            ...
            NCManageDatabase.shared.addE2eMetadata(account: domain.account,
                                                   serverUrl: serverUrl,
    ...
    func createSignature(metadata: E2eeV20.Metadata, users: [E2eeV20.Users]?, version: String, certificate: String, domain: NCDomain.Domain) -> String? {
        ...
        do {
            ...
            if let base64Data = base64.data(using: .utf8),
               let signatureData = NCEndToEndEncryption.shared().generateSignatureCMS(base64Data, certificate: certificate, privateKey: NCKeychain().getEndToEndPrivateKey(account: domain.account), userId: domain.userId) {
