...
class NCNetworkingE2EEDelete: NSObject {
    ...
    func delete(metadata: tableMetadata) async -> NKError {
        ...
        // DELETE FILE
        //
        let serverUrlFileName = self.utilityFileSystem.serverDirectoryDown(serverUrl: metadata.serverUrl, fileNameFolder: metadata.fileName)
        ...
        if result.error == .success || result.error.errorCode == NCGlobal.shared.errorResourceNotFound {
            do {
                ...
                if metadata.directory {
                    await self.database.deleteDirectoryAndSubDirectoryAsync(serverUrl: self.utilityFileSystem.serverDirectoryDown(serverUrl: metadata.serverUrl, fileNameFolder: metadata.fileName), account: metadata.account)
                }
            } catch { }
        } else {
