...
class NCAssistantTask: ObservableObject {
    ...
    func selectTask(_ task: NKTextProcessingTask) {
        ...
        NextcloudKit.shared.textProcessingGetTask(taskId: id, account: NCDomain.shared.getActiveAccount()) { _, task, _, error in
    ...
    func scheduleTask(input: String) {
        ...
        NextcloudKit.shared.textProcessingSchedule(input: input, typeId: selectedType?.id ?? "", identifier: "assistant", account: NCDomain.shared.getActiveAccount()) { _, task, _, error in
            self.isLoading = false
    ...
    func deleteTask(_ task: NKTextProcessingTask) {
        ...
        NextcloudKit.shared.textProcessingDeleteTask(taskId: id, account: NCDomain.shared.getActiveAccount()) { _, task, _, error in
            self.isLoading = false
    ...
    private func loadAllTypes() {
        ...
        NextcloudKit.shared.textProcessingGetTypes(account: NCDomain.shared.getActiveAccount()) { _, types, _, error in
            self.isLoading = false
    ...
    private func loadAllTasks(appId: String = "assistant") {
        ...
        NextcloudKit.shared.textProcessingTaskList(appId: appId, account: NCDomain.shared.getActiveAccount()) { _, tasks, _, error in
            self.isLoading = false
