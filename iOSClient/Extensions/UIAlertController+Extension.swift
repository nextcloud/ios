            ...
            if markE2ee {
                Task {
                    ...
                    if createFolderResults.error == .success {
                        let error = await NCNetworkingE2EECreateFolder.shared.markFolderE2ee(account: urlBase.account, fileName: fileNameFolder, serverUrl: serverUrl, userId: urlBase.userId)
                        if error != .success {
