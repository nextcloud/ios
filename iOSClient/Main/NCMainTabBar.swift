    ...
    private func createButtons() {
        ...
        centerButton.action(for: .touchUpInside) { _ in
            ...
            if let controller = self.window?.rootViewController as? NCMainTabBarController {
                ...
                if let directory = NCManageDatabase.shared.getTableDirectory(predicate: NSPredicate(format: "account == %@ AND serverUrl == %@", NCSession.shared.getActiveSession().account, serverUrl)) {
