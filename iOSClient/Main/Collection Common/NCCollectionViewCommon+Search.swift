...
extension NCCollectionViewCommon {
        ...
        case "fulltextsearch":
            for entry in searchResult.entries {
                ...
                if let metadata = await NCManageDatabase.shared.getMetadataAsync(
                    predicate: NSPredicate(format: "account == %@ && path == %@ && fileName == %@", session.account, NCGlobal.shared.davFilesBasePath + session.user + dir, filename)) {
                    metadatas.append(metadata)
                } else {
                    if let metadata = await loadMetadata(session: session,
                                                         provider: provider,
    ...
    private func loadMetadata(session: NCSession.Session,
        ...
        let urlPath = session.urlBase + NCGlobal.shared.davFilesBasePath + session.user + filePath
