...
class NCOffline: NCCollectionViewCommon {
    ...
    override func reloadDataSource() {
        ...
        if self.serverUrl.isEmpty {
            ...
            metadatas = self.database.getResultsMetadatasPredicate(NSPredicate(format: "account == %@ AND ocId IN %@ AND NOT (status IN %@)", session.account, ocIds, global.metadataStatusHideInView), layoutForView: layoutForView, directoryOnTop: directoryOnTop)
        } else {
