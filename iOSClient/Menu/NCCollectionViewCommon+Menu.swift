    ...
    func toggleMenu(metadata: tableMetadata, indexPath: IndexPath, imageIcon: UIImage?) {
        ...
        if metadata.canSetDirectoryAsE2EE {
            actions.append(
                NCMenuAction(
                    ...
                    icon: utility.loadImage(named: "lock", color: NCBrandColor.shared.iconImageColor),
                    order: 30,
                    action: { _ in
                        Task {
        ...
        if metadata.canOpenExternalEditor {
            ...
            if editors.contains(NCGlobal.shared.editorOnlyoffice) {
                ...
                icon = utility.loadImage(named: "onlyoffice", color: NCBrandColor.shared.iconImageColor)
            } else if isRichDocument {
                editor = NCGlobal.shared.editorCollabora
                title = NSLocalizedString("_open_in_collabora_", comment: "")
                icon = utility.loadImage(named: "collabora", color: NCBrandColor.shared.iconImageColor)
            }
        ...
        if self is NCFiles, metadata.directory {
            actions.append(
                NCMenuAction(
                    ...
                    icon: utility.loadImage(named: "paintpalette", color: NCBrandColor.shared.iconImageColor),
                    order: 160,
                    action: { _ in
