...
extension NCShare {
    func toggleShareMenu(for share: tableShare) {
        let capability = NCCapabilities.shared.capabilities[self.metadata.account]
        ...
        actions.append(
            NCMenuAction(
                ...
                action: { _ in
                    Task {
                        if share.shareType != NCShareCommon().SHARE_TYPE_LINK, let metadata = self.metadata, metadata.e2eEncrypted && capability?.capabilityE2EEApiVersion == NCGlobal.shared.e2eeVersionV20 {
