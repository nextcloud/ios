...
extension NCTrash {
    func toggleMenuMore(with objectId: String, image: UIImage?, isGridCell: Bool) {
        guard let tableTrash = NCManageDatabase.shared.getTrashItem(fileId: objectId, account: NCSession.shared.getSession(controller: tabBarController).account) else { return }
