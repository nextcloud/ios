...
extension NCMenuAction {
    ...
    static func share(selectedMetadatas: [tableMetadata], viewController: UIViewController, order: Int = 0, completion: (() -> Void)? = nil) -> NCMenuAction {
        NCMenuAction(
            ...
            order: order,
            action: { _ in
                NCActionCenter.shared.openActivityViewController(selectedMetadata: selectedMetadatas, viewController: viewController)
                completion?()
            }
        )
    }
    ...
    static func moveOrCopyAction(selectedMetadatas: [tableMetadata], viewController: UIViewController, order: Int = 0, completion: (() -> Void)? = nil) -> NCMenuAction {
        NCMenuAction(
            ...
            order: order,
            action: { _ in
                NCActionCenter.shared.openSelectView(items: selectedMetadatas, viewController: viewController)
                completion?()
            }
        )
    }
